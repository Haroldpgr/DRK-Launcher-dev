...
export class EjecutarFabric {
  ...
  async ejecutar(opts: FabricLaunchOptions): Promise<ChildProcess> {
    try {
      ...
      const loaderJarPath = path.join(opts.instancePath, 'loader', `fabric-loader-${opts.loaderVersion}.jar`);
      ...
      logProgressService.info(`[Fabric] Ejecutando Fabric...`);
      ...
      logProgressService.error(`[Fabric] Error al ejecutar:`, error);
      throw error;
    }
  }
