...
var downloadQueueService_exports = {};
__export(downloadQueueService_exports, {
  DownloadQueueService: () => DownloadQueueService,
  downloadQueueService: () => downloadQueueService
});
var import_node_path3, import_node_fs2, import_node_stream, import_node_util, import_node_fetch, pipelineAsync, DownloadQueueService, downloadQueueService;
var init_downloadQueueService = __esm({
  "src/services/downloadQueueService.ts"() {
    ...
    pipelineAsync = (0, import_node_util.promisify)(import_node_stream.pipeline);
    DownloadQueueService = class {
      downloads = /* @__PURE__ */ new Map();
      activeDownloads = /* @__PURE__ */ new Set();
      maxConcurrentDownloads = 3;
      // Reducir a 3 para evitar sobrecarga
      timeoutMs = 3e5;
      // Aumentar timeout a 5 minutos para descargas lentas
      /**
        ...
        try {
          ...
          const response = await (0, import_node_fetch.default)(download.url, {
            ...
            headers: {
              ...
              // Evitar compresión para poder medir progreso
      ...
      async calculateFileHash(filePath, algorithm) {
        ...
        return new Promise((resolve, reject) => {
          ...
          const input = fs15.createReadStream(filePath);
  ...
  "node_modules/node-stream-zip/node_stream_zip.js"(exports2, module2) {
    ...
    var StreamZip = function(config) {
      ...
      function extract(entry, outPath, callback) {
        that.stream(entry, (err, stm) => {
          if (err) {
            ...
            fs15.open(outPath, "w", (err2, fdFile) => {
              if (err2) {
              ...
              fsStm = fs15.createWriteStream(outPath, { fd: fdFile });
              fsStm.on("finish", () => {
      ...
      function createDirectories(baseDir, dirs, callback) {
        ...
        let dir = dirs.shift();
        dir = path16.join(baseDir, path16.join(...dir));
        fs15.mkdir(dir, { recursive: true }, (err) => {
      ...
      function extractFiles(baseDir, baseRelPath, files, callback, extractedCount) {
        ...
        const targetPath = path16.join(baseDir, file.name.replace(baseRelPath, ""));
      ...
      this.extract = function(entry, outPath, callback) {
        ...
        if (!entry || entry.isDirectory) {
          ...
          for (const e in entries) {
            if (Object.prototype.hasOwnProperty.call(entries, e) && e.lastIndexOf(entryName, 0) === 0) {
              ...
              if (childEntry.isFile) {
                ...
                relPath = path16.dirname(relPath);
              }
          ...
          fs15.stat(outPath, (err, stat) => {
            if (stat && stat.isDirectory()) {
              extract(entry, path16.join(outPath, path16.basename(entry.name)), callback);
            } else {
    ...
    var EntryDataReaderStream = class extends stream.Readable {
      ...
      _read(n) {
        ...
        if (buffer.length) {
          fs15.read(this.fd, buffer, 0, buffer.length, this.offset + this.pos, this.readCallback);
        } else {
          this.push(null);
        }
      }
...
function initDB() {
  try {
    ...
    const file = import_node_path2.default.join(drkLauncherDir, "drk.sqlite");
...
function isInstanceReady(instancePath) {
  console.log(`Verificando si la instancia en ${instancePath} est\xE1 lista para jugar...`);
  try {
    ...
    if (!import_node_fs4.default.existsSync(clientJarPath)) {
      console.log(`[VERIFICACI\xD3N] client.jar no encontrado en ${clientJarPath}`);
      try {
        const files = import_node_fs4.default.readdirSync(instancePath);
        console.log(`[VERIFICACI\xD3N] Archivos en la instancia ${instancePath}:`, files.join(", "));
      } catch (dirError) {
        console.log(`[VERIFICACI\xD3N] No se pudo leer el directorio de la instancia:`, dirError);
      }
      return false;
    }
    let clientJarStats;
    try {
      ...
      console.log(`[VERIFICACI\xD3N] No se pudo acceder al archivo client.jar: ${statError}`);
      return false;
    }
    if (clientJarStats.size < 1024 * 1024) {
      console.log(`[VERIFICACI\xD3N] client.jar es demasiado peque\xF1o (${clientJarStats.size} bytes), probablemente no est\xE9 completamente descargado`);
      return false;
    }
    console.log(`[VERIFICACI\xD3N] client.jar encontrado y v\xE1lido: ${clientJarPath} (${clientJarStats.size} bytes)`);
    ...
    if (!import_node_fs4.default.existsSync(launcherAssetsPath)) {
      console.log(`[VERIFICACI\xD3N] Carpeta de assets compartida no existe: ${launcherAssetsPath}`);
      return false;
    }
    const indexesPath = import_node_path5.default.join(launcherAssetsPath, "indexes");
    const objectsPath = import_node_path5.default.join(launcherAssetsPath, "objects");
    if (!import_node_fs4.default.existsSync(indexesPath)) {
      console.log(`[VERIFICACI\xD3N] Carpeta de \xEDndices de assets no existe: ${indexesPath}`);
      return false;
    }
    if (!import_node_fs4.default.existsSync(objectsPath)) {
      console.log(`[VERIFICACI\xD3N] Carpeta de objetos de assets no existe: ${objectsPath}`);
      return false;
    }
    try {
      const indexFiles = import_node_fs4.default.readdirSync(indexesPath);
      if (indexFiles.length === 0) {
        console.log(`[VERIFICACI\xD3N] Carpeta de \xEDndices de assets est\xE1 vac\xEDa: ${indexesPath}`);
        return false;
      }
    } catch (indexError) {
      console.log(`[VERIFICACI\xD3N] Error al leer carpeta de \xEDndices de assets:`, indexError);
      return false;
    }
    try {
      const objectFolders = import_node_fs4.default.readdirSync(objectsPath);
      if (objectFolders.length === 0) {
        console.log(`[VERIFICACI\xD3N] Carpeta de objetos de assets est\xE1 vac\xEDa: ${objectsPath}`);
        return false;
      }
    } catch (objectsError) {
      console.log(`[VERIFICACI\xD3N] Error al leer carpeta de objetos de assets:`, objectsError);
      return false;
    }
    ...
    for (const folder of requiredFolders) {
      ...
      if (!import_node_fs4.default.existsSync(folderPath)) {
        try {
          ...
          console.log(`[VERIFICACI\xD3N] Carpeta creada: ${folderPath}`);
        } catch (mkdirErr) {
          console.log(`[VERIFICACI\xD3N] No se pudo crear carpeta ${folder}:`, mkdirErr);
        }
      }
    }
    console.log(`[VERIFICACI\xD3N] Instancia en ${instancePath} est\xE1 lista para jugar`);
    return true;
  } catch (error) {
    console.error("[VERIFICACI\xD3N] Error al verificar si la instancia est\xE1 lista:", error);
    console.error("[VERIFICACI\xD3N] Error en detalle:", error.message);
    return false;
  }
}
  ...
  try {
    ...
    if (!import_node_fs4.default.existsSync(versionJsonPath2)) {
      ...
      if (import_node_fs4.default.existsSync(launcherAssetsPath)) {
        const indexesPath = import_node_path5.default.join(launcherAssetsPath, "indexes");
        const objectsPath = import_node_path5.default.join(launcherAssetsPath, "objects");
        if (import_node_fs4.default.existsSync(indexesPath) && import_node_fs4.default.existsSync(objectsPath)) {
          const indexesCount = import_node_fs4.default.readdirSync(indexesPath).length;
          const objectsSubdirs = import_node_fs4.default.readdirSync(objectsPath).filter(
            (dir) => import_node_fs4.default.statSync(import_node_path5.default.join(objectsPath, dir)).isDirectory()
          );
...
var JavaDetector = class {
  ...
  javaRegistryKeys = [];
  defaultJavaId = null;
  config = { defaultJavaId: null, installedJavas: [] };
  constructor() {
  }
  saveConfig() {
  ...
  async detectJava() {
    ...
    if (process.env.JAVA_HOME) {
      const javaHomePath = process.platform === "win32" ? (0, import_path.join)(process.env.JAVA_HOME, "bin", "java.exe") : (0, import_path.join)(process.env.JAVA_HOME, "bin", "java");
      if ((0, import_fs.existsSync)(javaHomePath)) {
        try {
  ...
  async detectJavaInWindows() {
    ...
    for (const basePath of commonPaths) {
      if ((0, import_fs.existsSync)(basePath)) {
        try {
          const folders = (0, import_fs.readdirSync)(basePath);
          for (const folder of folders) {
            const fullPath = (0, import_path.join)(basePath, folder, "bin", "java.exe");
    ...
    if ((0, import_fs.existsSync)(programFilesJavaPath)) {
      try {
        const subfolders = (0, import_fs.readdirSync)(programFilesJavaPath);
        for (const folder of subfolders) {
          const possibleJavaPaths = [
            (0, import_path.join)(programFilesJavaPath, folder, "bin", "java.exe"),
            (0, import_path.join)(programFilesJavaPath, folder, "bin", "javaw.exe"),
            (0, import_path.join)(programFilesJavaPath, folder, "jre", "bin", "java.exe")
          ];
          for (const javaPath of possibleJavaPaths) {
            if ((0, import_fs.existsSync)(javaPath)) {
              try {
    ...
    try {
      ...
      const javaRuntimeRegResults = (0, import_child_process.execSync)(`reg query "${javaRuntimeRegPath}" /s`, { encoding: "utf8" });
        ...
        for (const match of runtimeMatches) {
          ...
          if (version) {
            try {
              ...
              const result = (0, import_child_process.execSync)(versionQuery, { encoding: "utf8" });
              const homeMatch = result.match(/REG_SZ\s+(.+)/);
              if (homeMatch) {
                const javaHome = homeMatch[1].trim();
                const javaPath = (0, import_path.join)(javaHome, "bin", "java.exe");
                if ((0, import_fs.existsSync)(javaPath)) {
      ...
      const javaJdkRegResults = (0, import_child_process.execSync)(`reg query "${javaJdkRegPath}" /s`, { encoding: "utf8" });
  ...
  async getJavaVersionFromDirName(dirName) {
    const patterns = [
      /jdk(\d+)/i,
      // jdk8, jdk17, jdk21
      /jdk-(\d+)/i,
    ...
    for (const basePath of commonPaths) {
      if ((0, import_fs.existsSync)(basePath)) {
        try {
          const folders = (0, import_fs.readdirSync)(basePath);
          for (const folder of folders) {
            const fullPath = (0, import_path.join)(basePath, folder, "bin", "java");
            if ((0, import_fs.existsSync)(fullPath)) {
              const version = await this.getJavaVersion(fullPath);
              this.detectedJavas.push({
  ...
  async detectJavaInMacOS() {
    const commonPaths = [
      "/Library/Java/JavaVirtualMachines/",
      "/System/Library/Java/JavaVirtualMachines/",
      "/Users/" + import_os.default.userInfo().username + "/.sdkman/candidates/java/current/"
    ];
    for (const basePath of commonPaths) {
      if ((0, import_fs.existsSync)(basePath)) {
        try {
          const folders = (0, import_fs.readdirSync)(basePath);
          for (const folder of folders) {
            const fullPath = (0, import_path.join)(basePath, folder, "Contents", "Home", "bin", "java");
            if ((0, import_fs.existsSync)(fullPath)) {
              const version = await this.getJavaVersion(fullPath);
              this.detectedJavas.push({
  ...
  which(command) {
    try {
      if (process.platform === "win32") {
        const result = (0, import_child_process.execSync)(`where ${command}`, { encoding: "utf8" });
        return result.trim().split("\n")[0].trim();
      } else {
        const result = (0, import_child_process.execSync)(`which ${command}`, { encoding: "utf8" });
        return result.trim();
      }
    } catch (error) {
      return null;
  ...
  getJavaVersion(javaPath) {
    return new Promise((resolve, reject) => {
      const child = (0, import_child_process.spawn)(javaPath, ["-version"]);
      let output = "";
      let errorOutput = "";
...
var JavaService = class {
  ...
  async testJava(path16) {
    try {
      await this.detector.getJavaVersion(path16);
      return true;
    } catch {
      return false;
    }
...
var ModrinthDownloadService = class {
  ...
  async downloadContent(projectId, instancePath, mcVersion, loader, contentType = "mod", versionId) {
    try {
      ...
      switch (contentType) {
        case "mod":
          targetDir = import_node_path8.default.join(instancePath, "mods");
          break;
        case "resourcepack":
          targetDir = import_node_path8.default.join(instancePath, "resourcepacks");
          break;
        case "shader":
          targetDir = import_node_path8.default.join(instancePath, "shaderpacks");
          break;
        case "datapack":
          targetDir = import_node_path8.default.join(instancePath, "datapacks");
          break;
        default:
  ...
  async downloadModpack(projectId, instancePath, mcVersion, loader) {
    try {
      ...
      const tempDir = import_node_path8.default.join(instancePath, ".temp");
      this.ensureDir(tempDir);
      const tempPackPath = import_node_path8.default.join(tempDir, targetVersion.files[0]?.filename || "modpack.mrpack");
      ...
      if (import_node_fs7.default.existsSync(tempPackPath)) {
        import_node_fs7.default.unlinkSync(tempPackPath);
      }
      if (import_node_fs7.default.existsSync(tempDir)) {
        import_node_fs7.default.rmdirSync(tempDir, { recursive: true });
      }
   ...
   */
  findMatchingVersion(versions, mcVersion, loader) {
    let compatibleVersions = versions.filter(
      (version) => version.game_versions.includes(mcVersion)
    );
    if (loader) {
      compatibleVersions = compatibleVersions.filter(
        (version) => version.loaders.includes(loader.toLowerCase())
      );
    }
   ...
   */
  async processModpackManifest(manifest, instancePath, tempDir) {
    ...
    for (const file of files) {
      ...
      if (filePath.startsWith("mods/")) {
        ...
        targetDir = import_node_path8.default.join(instancePath, "config");
        isConfigFile = true;
      ...
      if (downloads && downloads.length > 0) {
        ...
        const targetPath = import_node_path8.default.join(targetDir, fileName);
...
var InstanceCreationService = class {
  ...
  async createFullInstance(name, version, loader = "vanilla", javaVersion = "17", maxMemory, minMemory, jvmArgs) {
    ...
    const instance = instanceService.createInstance({
      ...
      jvmArgs,
      id: void 0
      // The service will generate an ID if not provided
    });
    ...
    console.log("PASO 2.5: Verificando configuraci\xF3n de carpeta de assets...");
  ...
  async ensureAssetsAvailability(version) {
    ...
    try {
      ...
      console.log(`Descargando asset index ${assetIndex.id} para la versi\xF3n ${version}...`);
      await minecraftDownloadService.downloadVersionAssets(version);
      const launcherPath = getLauncherDataPath();
      const assetsPath = import_node_path9.default.join(launcherPath, "assets");
      const indexesPath = import_node_path9.default.join(assetsPath, "indexes");
      const objectsPath = import_node_path9.default.join(assetsPath, "objects");
      if (!import_node_fs8.default.existsSync(assetsPath)) {
        import_node_fs8.default.mkdirSync(assetsPath, { recursive: true });
        console.log(`Carpeta de assets creada: ${assetsPath}`);
      }
      if (!import_node_fs8.default.existsSync(indexesPath)) {
        import_node_fs8.default.mkdirSync(indexesPath, { recursive: true });
        console.log(`Carpeta de \xEDndices de assets creada: ${indexesPath}`);
      }
      if (!import_node_fs8.default.existsSync(objectsPath)) {
        import_node_fs8.default.mkdirSync(objectsPath, { recursive: true });
      ...
      console.log(`CONTINUANDO CREACI\xD3N DE INSTANCIA A\xDAN CON PROBLEMAS EN LA DESCARGA DE ASSETS`);
    }
  }
  /**
  ...
  async validateAssetsConfiguration(instancePath, version) {
    ...
    try {
      ...
      const indexesPath = import_node_path9.default.join(launcherAssetsPath, "indexes");
      const objectsPath = import_node_path9.default.join(launcherAssetsPath, "objects");
      if (!import_node_fs8.default.existsSync(indexesPath)) {
        throw new Error(`Carpeta de \xEDndices de assets no existe: ${indexesPath}`);
      }
      if (!import_node_fs8.default.existsSync(objectsPath)) {
        throw new Error(`Carpeta de objetos de assets no existe: ${objectsPath}`);
      }
      const indexFiles = import_node_fs8.default.readdirSync(indexesPath);
      if (indexFiles.length === 0) {
        console.warn(`Advertencia: No se encontraron archivos de \xEDndice de assets`);
      } else {
        console.log(`Se encontraron ${indexFiles.length} archivos de \xEDndice de assets`);
      }
      const objectFolders = import_node_fs8.default.readdirSync(objectsPath);
      if (objectFolders.length === 0) {
      ...
      console.log(`Configuraci\xF3n de assets validada para instancia en ${instancePath}`);
    } catch (error) {
      console.error(`Error al validar configuraci\xF3n de assets:`, error);
      throw error;
    }
  }
  /**
   ...
   */
  async setupJavaEnvironment(javaVersion) {
    try {
   ...
   */
  async downloadMinecraftBase(version) {
    try {
   ...
   */
  async downloadLoaderAndMerge(version, loader, instancePath) {
    if (loader === "fabric") {
   ...
   */
  async installContentToInstance(instancePath, contentId, contentType, mcVersion, loader) {
    if (!import_node_fs8.default.existsSync(instancePath)) {
...
var CurseForgeService = class {
  ...
  async downloadContent(projectId, instancePath, mcVersion, loader, contentType, versionId) {
    try {
  ...
  async downloadRegularContent(projectId, instancePath, mcVersion, loader, contentType, versionId) {
    const filesResponse = await (0, import_node_fetch6.default)(`${CURSEFORGE_API_URL}/mods/${projectId}/files`, {
      headers: this.headers
    });
    if (!filesResponse.ok) {
    ...
    let targetFile = null;
    if (versionId) {
      console.log(`Buscando archivo espec\xEDfico con ID: ${versionId} para el proyecto: ${projectId}`);
      targetFile = data.data.find((file) => file.id.toString() === versionId);
      if (!targetFile) {
        throw new Error(`No se encontr\xF3 la versi\xF3n espec\xEDfica con ID ${versionId} para el proyecto ${projectId}. Posibles IDs disponibles: ${data.data.slice(0, 5).map((f) => f.id).join(", ")}`);
      } else {
      ...
      let targetFile2 = null;
      const filesWithMCVersion = data.data.filter(
        (file) => file.gameVersions && Array.isArray(file.gameVersions) && file.gameVersions.some((v) => v.startsWith("1."))
      );
      if (filesWithMCVersion.length > 0) {
        const sortedByVersion = filesWithMCVersion.sort((a, b) => {
          const aVersions = a.gameVersions.filter((v) => v.startsWith("1."));
          const bVersions = b.gameVersions.filter((v) => v.startsWith("1."));
          if (aVersions.length === 0 && bVersions.length === 0) return 0;
          if (aVersions.length === 0) return 1;
          if (bVersions.length === 0) return -1;
          const aMaxVersion = this.extractVersionNumber(aVersions[0]);
          const bMaxVersion = this.extractVersionNumber(bVersions[0]);
          return bMaxVersion - aMaxVersion;
        });
        targetFile2 = sortedByVersion[0];
        console.log(`Archivo seleccionado basado en versi\xF3n m\xE1s reciente de Minecraft: ID ${targetFile2.id}, Nombre: ${targetFile2.fileName || targetFile2.displayName}, Versiones de Minecraft: ${targetFile2.gameVersions?.join(", ")}`);
      } else {
        const sortedByFileId = [...data.data].sort((a, b) => b.id - a.id);
        targetFile2 = sortedByFileId[0];
        console.log(`Archivo seleccionado basado en ID m\xE1s alto (m\xE1s reciente): ID ${targetFile2.id}, Nombre: ${targetFile2.fileName || targetFile2.displayName}`);
      }
    }
    if (!targetFile) {
    ...
    let targetDir = "";
    if (contentType === "mod") {
      targetDir = import_node_path10.default.join(instancePath, "mods");
    } else if (contentType === "resourcepack") {
      targetDir = import_node_path10.default.join(instancePath, "resourcepacks");
    } else if (contentType === "shader") {
      targetDir = import_node_path10.default.join(instancePath, "shaderpacks");
    } else if (contentType === "datapack") {
      targetDir = import_node_path10.default.join(instancePath, "datapacks");
    } else {
      targetDir = import_node_path10.default.join(instancePath, "mods");
    }
    if (!import_node_fs9.default.existsSync(targetDir)) {
    ...
    const filePath = import_node_path10.default.join(targetDir, fileName);
    const response = await (0, import_node_fetch6.default)(downloadUrl);
    if (!response.ok) {
      throw new Error(`Failed to download file: ${response.statusText}`);
    }
    const buffer = Buffer.from(await response.arrayBuffer());
    import_node_fs9.default.writeFileSync(filePath, buffer);
    console.log(`Downloaded and saved ${fileName} to ${filePath}`);
  }
  // Download a CurseForge modpack to an instance
  async downloadModpack(projectId, instancePath, mcVersion, loader, versionId) {
    try {
      const filesResponse = await (0, import_node_fetch6.default)(`${CURSEFORGE_API_URL}/mods/${projectId}/files`, {
        headers: this.headers
      });
      ...
      if (versionId) {
        ...
        const filteredFiles = data.data.filter((file) => {
          ...
          let hasLoader = true;
          if (loader) {
            const loaderInGameVersions = file.gameVersions.some(
              (v) => v.toLowerCase().includes(loader.toLowerCase())
            );
      ...
      const tempDir = import_node_path10.default.join(instancePath, ".temp");
      if (!import_node_fs9.default.existsSync(tempDir)) {
      ...
      const tempFilePath = import_node_path10.default.join(tempDir, fileName);
      const response = await (0, import_node_fetch6.default)(downloadUrl);
      if (!response.ok) {
        throw new Error(`Failed to download modpack: ${response.statusText}`);
      }
      const buffer = Buffer.from(await response.arrayBuffer());
      import_node_fs9.default.writeFileSync(tempFilePath, buffer);
      console.log(`Downloaded modpack ${fileName} to ${tempFilePath}`);
      await this.extractModpack(tempFilePath, instancePath);
      import_node_fs9.default.unlinkSync(tempFilePath);
      if (import_node_fs9.default.readdirSync(tempDir).length === 0) {
        import_node_fs9.default.rmdirSync(tempDir);
      }
  ...
  async extractModpack(zipPath, instancePath) {
    const nodeStreamZip = require_node_stream_zip();
    return new Promise((resolve, reject) => {
      const zip = new nodeStreamZip({
        file: zipPath,
        ...
        try {
          ...
          for (const entryName of entries) {
            ...
            if (entryName.startsWith("mods/")) {
              targetPath = import_node_path10.default.join(instancePath, entryName);
              ...
              if (entryName.endsWith(".jar") || entryName.endsWith(".zip")) {
                targetPath = import_node_path10.default.join(instancePath, "mods", import_node_path10.default.basename(entryName));
              } else {
                targetPath = import_node_path10.default.join(instancePath, entryName);
              }
            }
            ensureDir3(import_node_path10.default.dirname(targetPath));
            zip.extract(entryName, targetPath, (err) => {
              if (err) {
...
var VersionService = class {
  ...
  constructor() {
    const launcherPath = getLauncherDataPath();
    this.versionsCachePath = import_node_path11.default.join(launcherPath, "versions_cache.json");
  }
  /**
   ...
   */
  async getMinecraftVersions(forceRefresh = false) {
    if (!forceRefresh && await this.isCacheValid()) {
      try {
        const cachedData = JSON.parse(import_node_fs10.default.readFileSync(this.versionsCachePath, "utf-8"));
        console.log(`Cach\xE9 de versiones v\xE1lida, usando datos cacheados (${cachedData.versions.length} versiones)`);
        const releaseVersions2 = cachedData.versions.filter((v) => v.type === "release");
        return releaseVersions2;
      } catch (error) {
        console.error("Error al leer la cach\xE9 de versiones:", error);
      }
    }
   ...
   */
  async isCacheValid() {
    if (!import_node_fs10.default.existsSync(this.versionsCachePath)) {
      return false;
    }
    const stats = import_node_fs10.default.statSync(this.versionsCachePath);
    const now = Date.now();
    ...
    try {
      ...
      if (import_node_fs10.default.existsSync(this.versionsCachePath)) {
        try {
          ...
          return cachedData.versions.filter((v) => v.type === "release");
        } catch (cacheError) {
          console.error("Error al usar cach\xE9 como fallback:", cacheError);
        }
      }
      throw error;
    }
  ...
  async saveToCache(versions) {
    try {
      ...
      import_node_fs10.default.writeFileSync(this.versionsCachePath, JSON.stringify(cacheData, null, 2));
      console.log(`Cach\xE9 de versiones guardada con ${versions.length} entradas`);
    } catch (error) {
      console.error("Error al guardar cach\xE9 de versiones:", error);
    }
  }
  /**
  ...
  async getVersionDetails(version) {
    try {
      ...
      console.error(`Error al obtener detalles de la versi\xF3n ${version.id}:`, error);
      throw error;
    }
  }
  /**
   ...
   */
  async isVersionCompatibleWithLoader(versionId, loader) {
    return true;
  }
};
...
var EnhancedInstanceCreationService = class {
  ...
  constructor() {
    this.basePath = import_node_path12.default.join(getLauncherDataPath(), "instances");
    this.ensureDir(this.basePath);
  }
  /**
   * Asegura que un directorio exista, creándolo si es necesario
  ...
  getStatusDescription(status) {
    switch (status) {
      case "downloading_jre" /* DOWNLOADING_JRE */:
        return "Verificando/Descargando JRE";
      case "creating_structure" /* CREATING_STRUCTURE */:
        return "Creando estructura de instancia";
      case "downloading_version_metadata" /* DOWNLOADING_VERSION_METADATA */:
        return "Descargando metadatos de la versi\xF3n";
      case "downloading_libraries" /* DOWNLOADING_LIBRARIES */:
        return "Descargando bibliotecas";
      case "downloading_client" /* DOWNLOADING_CLIENT */:
        return "Descargando cliente";
      case "downloading_assets" /* DOWNLOADING_ASSETS */:
        return "Descargando assets";
      case "installing_loader" /* INSTALLING_LOADER */:
        return "Instalando mod loader";
      case "verifying_integrity" /* VERIFYING_INTEGRITY */:
   ...
   */
  async createInstance(config, instanceId) {
    ...
    this.updateProgress(progressId, "pending" /* PENDING */, 0, 10, "Iniciando creaci\xF3n de instancia");
    try {
      console.log(`Iniciando creaci\xF3n de instancia: ${config.name} (${config.version}) con ${config.loader || "vanilla"}`);
      this.updateProgress(progressId, "downloading_jre" /* DOWNLOADING_JRE */, 1, 10, "Verificando Java Runtime Environment");
      const javaPath = await javaDownloadService.getJavaForMinecraftVersion(config.version);
      if (this.isCancelled(progressId)) {
        throw new Error("Creaci\xF3n de instancia cancelada por el usuario");
      }
      this.updateProgress(progressId, "creating_structure" /* CREATING_STRUCTURE */, 2, 10, "Creando estructura de archivos");
      const instancePath = config.instancePath || import_node_path12.default.join(this.basePath, id);
      const instanceConfig = await this.createInstanceStructure(config, instancePath, id, javaPath);
      if (this.isCancelled(progressId)) {
        throw new Error("Creaci\xF3n de instancia cancelada por el usuario");
      }
      this.updateProgress(progressId, "downloading_version_metadata" /* DOWNLOADING_VERSION_METADATA */, 3, 10, "Descargando metadatos de la versi\xF3n");
      await versionService.getVersionDetails(await versionService.getVersionById(config.version));
      if (this.isCancelled(progressId)) {
        throw new Error("Creaci\xF3n de instancia cancelada por el usuario");
      }
      this.updateProgress(progressId, "downloading_libraries" /* DOWNLOADING_LIBRARIES */, 4, 10, "Descargando bibliotecas del juego");
      await minecraftDownloadService.downloadVersionLibraries(config.version);
      if (this.isCancelled(progressId)) {
        throw new Error("Creaci\xF3n de instancia cancelada por el usuario");
      }
      this.updateProgress(progressId, "downloading_client" /* DOWNLOADING_CLIENT */, 5, 10, "Descargando cliente de Minecraft");
      ...
      this.updateProgress(progressId, "verifying_integrity" /* VERIFYING_INTEGRITY */, 8, 10, "Verificando integridad de archivos");
      await this.verifyInstanceIntegrity(instancePath, config.version);
      if (this.isCancelled(progressId)) {
        throw new Error("Creaci\xF3n de instancia cancelada por el usuario");
      }
      this.updateProgress(progressId, "completed" /* COMPLETED */, 10, 10, "Instancia creada exitosamente");
      instanceConfig.ready = true;
      this.saveInstanceConfig(instancePath, instanceConfig);
      console.log(`Instancia ${config.name} (ID: ${id}) creada exitosamente en ${instancePath}`);
      return instanceConfig;
    } catch (error) {
      this.updateProgress(progressId, "error" /* ERROR */, 0, 10, `Error: ${error.message}`);
   ...
   */
  async createInstanceStructure(config, instancePath, id, javaPath) {
    this.ensureDir(instancePath);
    const requiredFolders = [
      "mods",
      // Mods de juego
    ...
    requiredFolders.forEach((folder) => {
      const folderPath = import_node_path12.default.join(instancePath, folder);
      this.ensureDir(folderPath);
    });
    const instanceConfig = {
      id,
   ...
   */
  saveInstanceConfig(instancePath, config) {
    const configPath = import_node_path12.default.join(instancePath, "instance.json");
   ...
   */
  async installLoader(loader, mcVersion, loaderVersion, instancePath) {
   ...
   * Instala Fabric
   */
  async installFabric(mcVersion, loaderVersion, instancePath) {
    try {
      const fabricApiUrl = `https://meta.fabricmc.net/v2/versions/loader/${mcVersion}`;
      const response = await fetch(fabricApiUrl);
      const fabricVersions = await response.json();
      const fabricEntry = fabricVersions.find((v) => v.loader.stable) || fabricVersions[0];
      if (!fabricEntry) {
        throw new Error(`No se encontr\xF3 versi\xF3n compatible de Fabric para Minecraft ${mcVersion}`);
      }
      ...
      const loaderDir = import_node_path12.default.join(instancePath, "loader");
      ...
      const fabricLoaderPath = import_node_path12.default.join(loaderDir, `fabric-loader-${finalLoaderVersion}.jar`);
      const responseLoader = await fetch(fabricLoaderUrl);
      if (!responseLoader.ok) {
        throw new Error(`Error al descargar Fabric Loader: ${responseLoader.status} ${responseLoader.statusText}`);
      }
      const buffer = Buffer.from(await responseLoader.arrayBuffer());
      import_node_fs11.default.writeFileSync(fabricLoaderPath, buffer);
  ...
  async installForge(mcVersion, loaderVersion, instancePath) {
    try {
      ...
      const forgeUniversalPath = import_node_path12.default.join(loaderDir, `forge-${forgeVersion}-universal.jar`);
      ...
      if (responseUniversal.ok) {
        ...
        import_node_fs11.default.writeFileSync(forgeUniversalPath, buffer);
        ...
        const forgeInstallerPath = import_node_path12.default.join(loaderDir, `forge-${forgeVersion}-installer.jar`);
        ...
        import_node_fs11.default.writeFileSync(forgeInstallerPath, buffer);
   ...
   * Instala Quilt
   */
  async installQuilt(mcVersion, loaderVersion, instancePath) {
    try {
      ...
      const quiltVersions = await response.json();
      if (!quiltVersions || quiltVersions.length === 0) {
        throw new Error(`No se encontr\xF3 versi\xF3n de Quilt para Minecraft ${mcVersion}`);
      }
      const quiltEntry = quiltVersions[0];
      ...
      const quiltLoaderPath = import_node_path12.default.join(loaderDir, `quilt-loader-${finalLoaderVersion}.jar`);
      const responseLoader = await fetch(quiltLoaderUrl);
      if (!responseLoader.ok) {
        throw new Error(`Error al descargar Quilt Loader: ${responseLoader.status} ${responseLoader.statusText}`);
      }
      const buffer = Buffer.from(await responseLoader.arrayBuffer());
      import_node_fs11.default.writeFileSync(quiltLoaderPath, buffer);
      console.log(`Quilt Loader descargado en: ${quiltLoaderPath}`);
    } catch (error) {
   ...
   * Instala NeoForge
   */
  async installNeoForge(mcVersion, loaderVersion, instancePath) {
    try {
      ...
      if (!neoForgeVersion) {
        ...
        const versions = await response.json();
        const compatibleVersions = versions.filter((v) => v.version.startsWith(`${mcVersion}-`));
        if (compatibleVersions.length > 0) {
          neoForgeVersion = compatibleVersions[0].version;
        }
      }
      if (!neoForgeVersion) {
        throw new Error(`No se encontr\xF3 versi\xF3n compatible de NeoForge para Minecraft ${mcVersion}`);
      }
      ...
      const neoForgePath = import_node_path12.default.join(loaderDir, `neoforge-${neoForgeVersion}.jar`);
      ...
      import_node_fs11.default.writeFileSync(neoForgePath, buffer);
   ...
   */
  async verifyInstanceIntegrity(instancePath, mcVersion) {
    console.log(`Verificando integridad de la instancia en ${instancePath} para Minecraft ${mcVersion}`);
    const clientJarPath = import_node_path12.default.join(instancePath, "client.jar");
    if (!import_node_fs11.default.existsSync(clientJarPath)) {
      throw new Error(`client.jar no encontrado en ${clientJarPath}`);
    }
    const stats = import_node_fs11.default.statSync(clientJarPath);
    if (stats.size < 1024 * 1024) {
      throw new Error(`client.jar tiene tama\xF1o inusualmente peque\xF1o: ${stats.size} bytes`);
    }
    const requiredFolders = ["mods", "config", "saves", "logs", "resourcepacks", "shaderpacks"];
    for (const folder of requiredFolders) {
      const folderPath = import_node_path12.default.join(instancePath, folder);
      if (!import_node_fs11.default.existsSync(folderPath)) {
        throw new Error(`Carpeta requerida no encontrada: ${folderPath}`);
      }
    }
    const configPath = import_node_path12.default.join(instancePath, "instance.json");
    if (!import_node_fs11.default.existsSync(configPath)) {
      throw new Error(`Archivo de configuraci\xF3n no encontrado: ${configPath}`);
    }
    try {
      const configContent = import_node_fs11.default.readFileSync(configPath, "utf-8");
      const config = JSON.parse(configContent);
      if (!config.version || config.version !== mcVersion) {
        throw new Error(`Versi\xF3n de la instancia no coincide con la versi\xF3n esperada`);
      }
    } catch (error) {
      throw new Error(`Archivo de configuraci\xF3n da\xF1ado o inv\xE1lido: ${error}`);
    }
    console.log(`Verificaci\xF3n de integridad completada para la instancia en ${instancePath}`);
  }
  /**
...
var LogProgressService = class {
  logsPath;
  logEvents = [];
  progressStatuses = /* @__PURE__ */ new Map();
  maxLogEntries = 1e3;
  // Limitar número de entradas para evitar consumir demasiada memoria
  constructor() {
    this.logsPath = import_node_path13.default.join(getLauncherDataPath(), "logs");
   ...
   */
  log(type, message, details, target) {
    const logEvent = {
      id: this.generateId(),
      timestamp: Date.now(),
      type,
    ...
    if (type === "progress" && typeof details === "object" && details.progress !== void 0) {
   ...
   */
  logDownloadProgress(downloadInfo) {
    const progress = downloadInfo.totalBytes && downloadInfo.totalBytes > 0 ? (downloadInfo.downloadedBytes || 0) / downloadInfo.totalBytes : 0;
   ...
   */
  logInstanceCreationProgress(instanceId, progress) {
  ...
  getLogsByType(type, count = 50) {
    return this.logEvents.filter((log) => log.type === type).slice(-count);
  }
  /**
  ...
  saveLogsToFile(filename) {
    ...
    const logFilePath = import_node_path13.default.join(this.logsPath, logFilename);
    const logData = {
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      logs: this.getRecentLogs(1e3)
      // Guardar los últimos 1000 eventos
    };
    import_node_fs12.default.writeFileSync(logFilePath, JSON.stringify(logData, null, 2));
    return logFilePath;
  }
  /**
  ...
  getOverallProgress() {
    ...
    if (hasErrors) {
      ...
      statusText = "\xA1Completado!";
    }
    return {
      progress: Math.min(overallProgress, 1),
      statusText,
      activeOperations: allProgress.filter(
        (status) => status.status === "in-progress" || status.status === "pending"
      ).length
    };
  }
};
var logProgressService = new LogProgressService();
...
var GameLaunchService = class {
  ...
  async validateInstanceFiles(opts) {
    ...
    const clientJarPath = import_node_path14.default.join(opts.instancePath, "client.jar");
    if (!import_node_fs13.default.existsSync(clientJarPath)) {
      throw new Error(`client.jar no encontrado en ${clientJarPath}`);
    }
    const clientStats = import_node_fs13.default.statSync(clientJarPath);
    if (clientStats.size < 1024 * 1024) {
      throw new Error(`client.jar tiene tama\xF1o muy peque\xF1o (${clientStats.size} bytes), probablemente incompleto`);
    }
    const assetsDir = import_node_path14.default.join(getLauncherDataPath(), "assets");
    if (!import_node_fs13.default.existsSync(assetsDir)) {
    ...
    if (!import_node_fs13.default.existsSync(launcherVersionJsonPath)) {
      throw new Error(`Archivo de metadatos de versi\xF3n no encontrado: ${launcherVersionJsonPath}`);
    }
    const versionData = JSON.parse(import_node_fs13.default.readFileSync(launcherVersionJsonPath, "utf-8"));
    const assetIndexId = versionData.assetIndex?.id || opts.mcVersion;
    const assetIndexPath = import_node_path14.default.join(getLauncherDataPath(), "assets", "indexes", `${assetIndexId}.json`);
    if (!import_node_fs13.default.existsSync(assetIndexPath)) {
      throw new Error(`\xCDndice de assets no encontrado: ${assetIndexPath}`);
    }
      ...
      for (const lib of versionData.libraries) {
        ...
        if (libraryAllowed && lib.downloads && lib.downloads.artifact) {
          ...
          if (lib.downloads.artifact.path) {
            libPath = import_node_path14.default.join(getLauncherDataPath(), "libraries", lib.downloads.artifact.path);
          } else {
            const nameParts = lib.name.split(":");
            const [group, artifact, version] = nameParts;
            const parts = group.split(".");
            libPath = import_node_path14.default.join(getLauncherDataPath(), "libraries", ...parts, artifact, version, `${artifact}-${version}.jar`);
          }
          if (!import_node_fs13.default.existsSync(libPath)) {
            logProgressService.info(`Descargando librer\xEDa faltante: ${lib.name}`, {
              path: libPath,
              library: lib.name
            });
            this.ensureDir(import_node_path14.default.dirname(libPath));
            try {
            ...
            try {
              const libStats = import_node_fs13.default.statSync(libPath);
  ...
  async downloadLibrary(url, outputPath) {
    try {
      ...
      const buffer = Buffer.from(await response.arrayBuffer());
      import_node_fs13.default.writeFileSync(outputPath, buffer);
      return true;
    } catch (error) {
  ...
  async launchGame(opts) {
    try {
      ...
      child.on("close", (code) => {
        logProgressService.info(`Minecraft cerrado con c\xF3digo: ${code}`, {
          instance: opts.instanceConfig.name,
          exitCode: code
        });
        opts.onExit?.(code);
   ...
   */
  async buildLaunchArguments(opts) {
    ...
    if (loader === "vanilla") {
      ...
      return await this.buildArgumentsForModded(opts, loader, jvmArgs);
    }
  }
  /**
  ...
  async buildArgumentsForVanilla(opts, jvmArgs) {
    ...
    let classpath = import_node_path14.default.join(opts.instancePath, "client.jar");
    let mainClass = "net.minecraft.client.main.Main";
    if (import_node_fs13.default.existsSync(versionJsonPath2)) {
      try {
        const versionData = JSON.parse(import_node_fs13.default.readFileSync(versionJsonPath2, "utf-8"));
          ...
          for (const lib of versionData.libraries) {
            ...
            if (lib.downloads && lib.downloads.artifact) {
              ...
              if (lib.downloads.artifact.path) {
                ...
                libPath = import_node_path14.default.join(getLauncherDataPath(), "libraries", this.getLibraryPath(lib.name));
              }
              const possiblePaths = [
                libPath,
                // Ruta estándar en .DRK Launcher/libraries
                import_node_path14.default.join(opts.instancePath, "libraries", this.getLibraryPath(lib.name)),
                // En la instancia
                import_node_path14.default.join(process.env.APPDATA || "", ".minecraft", "libraries", this.getLibraryPath(lib.name))
                // .minecraft estándar
              ];
              let found = false;
              for (const possiblePath of possiblePaths) {
                if (import_node_fs13.default.existsSync(possiblePath)) {
              ...
              if (!found) {
                logProgressService.warning(`Librer\xEDa no encontrada: ${lib.name}. Intentando descargar...`);
                try {
                  if (lib.downloads.artifact.url) {
                    const libDir = import_node_path14.default.dirname(libPath);
                    import_node_fs13.default.mkdirSync(libDir, { recursive: true });
                    const downloadResult = await this.downloadLibrary(lib.downloads.artifact.url, libPath);
                    if (downloadResult && import_node_fs13.default.existsSync(libPath)) {
                      libraryJars.push(libPath);
                      logProgressService.success(`Librer\xEDa descargada: ${lib.name}`);
                      found = true;
                    }
                  }
                } catch (downloadError) {
                  logProgressService.error(`Error al descargar la librer\xEDa ${lib.name}: ${downloadError}`);
                }
                if (!found) {
                  logProgressService.warning(`No se pudo descargar la librer\xEDa: ${lib.name}`);
                }
              }
            } else if (lib.url) {
              const libPath = import_node_path14.default.join(getLauncherDataPath(), "libraries", this.getLibraryPath(lib.name));
              const possiblePaths = [
                libPath,
                import_node_path14.default.join(opts.instancePath, "libraries", this.getLibraryPath(lib.name)),
                import_node_path14.default.join(process.env.APPDATA || "", ".minecraft", "libraries", this.getLibraryPath(lib.name))
              ...
              if (!found) {
                logProgressService.warning(`Librer\xEDa no encontrada (formato viejo): ${lib.name}. Intentando descargar...`);
                try {
                  const libDir = import_node_path14.default.dirname(libPath);
                  import_node_fs13.default.mkdirSync(libDir, { recursive: true });
                  ...
                  if (downloadResult && import_node_fs13.default.existsSync(libPath)) {
                    libraryJars.push(libPath);
                    logProgressService.success(`Librer\xEDa (formato viejo) descargada: ${lib.name}`);
                    found = true;
                  }
                } catch (downloadError) {
                  logProgressService.error(`Error al descargar la librer\xEDa (formato viejo) ${lib.name}: ${downloadError}`);
                }
                if (!found) {
                  logProgressService.warning(`No se pudo descargar la librer\xEDa (formato viejo): ${lib.name}`);
                }
              }
            }
          }
        }
        const clientJar = import_node_path14.default.join(opts.instancePath, "client.jar");
        if (import_node_fs13.default.existsSync(clientJar)) {
          libraryJars.push(clientJar);
        }
        if (libraryJars.length > 0) {
          classpath = libraryJars.join(import_node_path14.default.delimiter);
        } else {
          logProgressService.warning(`No se encontraron librer\xEDas para la versi\xF3n ${opts.mcVersion}, usando client.jar como fallback`);
        }
      } catch (error) {
        logProgressService.error("Error al leer archivo de versi\xF3n para construir classpath:", error);
        classpath = import_node_path14.default.join(opts.instancePath, "client.jar");
      ...
      for (const libName of commonLibs) {
        const libPath = import_node_path14.default.join(getLauncherDataPath(), "libraries", this.getLibraryPath(libName));
        const possiblePaths = [
          libPath,
          import_node_path14.default.join(opts.instancePath, "libraries", this.getLibraryPath(libName)),
          import_node_path14.default.join(process.env.APPDATA || "", ".minecraft", "libraries", this.getLibraryPath(libName))
        ];
        let found = false;
        for (const possiblePath of possiblePaths) {
          if (import_node_fs13.default.existsSync(possiblePath)) {
        ...
        if (!found) {
          logProgressService.warning(`Librer\xEDa com\xFAn no encontrada: ${libName}`);
        }
      }
      if (libraryJars.length > 0) {
        classpath = [import_node_path14.default.join(opts.instancePath, "client.jar"), ...libraryJars].join(import_node_path14.default.delimiter);
      }
    }
    ...
    if (import_node_fs13.default.existsSync(moddedVersionJsonPath)) {
      try {
        const versionData = JSON.parse(import_node_fs13.default.readFileSync(moddedVersionJsonPath, "utf-8"));
    ...
    const gameArgs = [
      ...
      opts.instancePath,
      "--assetsDir",
      import_node_path14.default.join(getLauncherDataPath(), "assets"),
  ...
  async buildArgumentsForModded(opts, loader, jvmArgs) {
    ...
    if (loaderJarPath && import_node_fs13.default.existsSync(loaderJarPath)) {
    ...
    if (import_node_fs13.default.existsSync(versionJsonPath)) {
      try {
        const versionData = JSON.parse(import_node_fs13.default.readFileSync(versionJsonPath, "utf-8"));
        if (versionData.libraries && Array.isArray(versionData.libraries)) {
          for (const lib of versionData.libraries) {
            if (lib.downloads && lib.downloads.artifact) {
              let libPath;
              if (lib.downloads.artifact.path) {
                libPath = import_node_path14.default.join(launcherPath, "libraries", lib.downloads.artifact.path);
              } else {
                libPath = import_node_path14.default.join(launcherPath, "libraries", this.getLibraryPath(lib.name));
              }
              const possiblePaths = [
                libPath,
                // Ruta estándar en .DRK Launcher/libraries
    ...
    const classpath = libraryJars.join(import_node_path14.default.delimiter);
    const fakeUUID = ensureValidUUID(opts.userProfile?.id);
    let assetIndexId = opts.mcVersion;
    const loaderVersionJsonPath = await minecraftDownloadService.downloadVersionMetadata(opts.mcVersion);
    if (import_node_fs13.default.existsSync(loaderVersionJsonPath)) {
      try {
        const versionData = JSON.parse(import_node_fs13.default.readFileSync(loaderVersionJsonPath, "utf-8"));
        if (versionData.assetIndex && versionData.assetIndex.id) {
          assetIndexId = versionData.assetIndex.id;
        }
      } catch (error) {
  ...
  logOutputToFile(instanceName, output) {
    try {
      const launcherPath = getLauncherDataPath();
      const logsDir = import_node_path14.default.join(launcherPath, "logs", "games");
      import_node_fs13.default.mkdirSync(logsDir, { recursive: true });
      const safeInstanceName = instanceName.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").trim();
      const logFilePath = import_node_path14.default.join(logsDir, `${safeInstanceName}_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.log`);
      const timestamp = (/* @__PURE__ */ new Date()).toISOString();
      const logEntry = `[${timestamp}] ${output}
`;
      import_node_fs13.default.appendFileSync(logFilePath, logEntry);
    } catch (error) {
  ...
  getLibraryPath(libraryName) {
    const [group, artifact, version] = libraryName.split(":");
    const parts = group.split(".");
    return import_node_path14.default.join(...parts, artifact, version, `${artifact}-${version}.jar`);
  }
  /**
   ...
   */
  findLoaderJar(instancePath, loader, version) {
    const loaderDir = import_node_path14.default.join(instancePath, "loader");
    if (import_node_fs13.default.existsSync(loaderDir)) {
      const files = import_node_fs13.default.readdirSync(loaderDir);
      for (const file of files) {
        if (file.endsWith(".jar")) {
          if (file.toLowerCase().includes(loader)) {
            return import_node_path14.default.join(loaderDir, file);
          }
        }
      }
    }
    if (import_node_fs13.default.existsSync(instancePath)) {
      const instanceFiles = import_node_fs13.default.readdirSync(instancePath);
      for (const file of instanceFiles) {
        if (file.endsWith(".jar")) {
          if (file.toLowerCase().includes(loader)) {
            return import_node_path14.default.join(instancePath, file);
          }
        }
      }
    }
    const standardMinecraftPath = import_node_path14.default.join(process.env.APPDATA || "", ".minecraft", "versions");
    const loaderVersionPath = import_node_path14.default.join(standardMinecraftPath, `${loader}-${version || ""}`);
    if (import_node_fs13.default.existsSync(loaderVersionPath)) {
      const versionFiles = import_node_fs13.default.readdirSync(loaderVersionPath);
      for (const file of versionFiles) {
        if (file.endsWith(".jar") && file.includes(loader)) {
          return import_node_path14.default.join(loaderVersionPath, file);
        }
      }
    }
    return null;
...
async function createWindow() {
  ...
  if (url) {
    ...
    await win.loadFile(import_node_path15.default.join(process.cwd(), "dist", "index.html"));
  }
}
function basePaths() {
  const data = getUserDataPath();
  const settingsPath = import_node_path15.default.join(data, "settings.json");
  const drkLauncherDir = data;
  ensureDir2(drkLauncherDir);
  const instancesDir = import_node_path15.default.join(drkLauncherDir, "instances");
  ensureDir2(instancesDir);
  return {
    data,
    settingsPath,
    instancesBaseDefault: instancesDir,
    // Instancias en la subcarpeta instances/
...
function readJSON(file, fallback) {
  try {
    ...
    const raw = import_node_fs14.default.readFileSync(file, "utf-8");
...
function writeJSON(file, value) {
  ensureDir2(import_node_path15.default.dirname(file));
  import_node_fs14.default.writeFileSync(file, JSON.stringify(value, null, 2));
}
function settings() {
...
function instancesFile() {
  ...
  return import_node_path15.default.join(data, "instances.json");
}
...
function deleteInstance(id) {
  ...
  if (item && import_node_fs14.default.existsSync(item.path)) import_node_fs14.default.rmSync(item.path, { recursive: true, force: true });
}
...
function crashesFile() {
  const { data } = basePaths();
  return import_node_path15.default.join(data, "crashes.json");
}
...
import_electron2.ipcMain.handle("crash:analyze", async (_e, p) => {
  ...
  if (!i) return null;
  const target = p.logPath || import_node_path15.default.join(i.path, "logs", "latest.log");
  if (!import_node_fs14.default.existsSync(target)) return null;
  const txt = import_node_fs14.default.readFileSync(target, "utf-8");
...
import_electron2.ipcMain.handle("crash:delete", async (_, id) => {
  ...
  if (deleted) {
    ...
    if (crash && crash.logPath && import_node_fs14.default.existsSync(crash.logPath)) {
      try {
        import_node_fs14.default.unlinkSync(crash.logPath);
      } catch (error) {
...
import_electron2.ipcMain.handle("logs:readLog", async (_e, logPath) => {
  try {
    const { data } = basePaths();
    const instancesPath = import_node_path15.default.join(data, "instances");
    const resolvedPath = import_node_path15.default.resolve(logPath);
    const resolvedInstancesPath = import_node_path15.default.resolve(instancesPath);
    ...
    const content = import_node_fs14.default.readFileSync(resolvedPath, "utf-8");
    return content;
  } catch (error) {
...
import_electron2.ipcMain.handle("servers:list", async () => {
  const { data } = basePaths();
  const file = import_node_path15.default.join(data, "servers.json");
...
import_electron2.ipcMain.handle("game:launch", async (_e, p) => {
  ...
  try {
    ...
    if (!isInstanceReady(i.path)) {
      const errorMsg = `La instancia ${i.name} no est\xE1 lista para jugar. Archivos esenciales faltantes.`;
      logProgressService.error(errorMsg);
      throw new Error(errorMsg);
    }
    const assetsReady = areAssetsReadyForVersion(i.path, i.version);
    if (!assetsReady) {
      logProgressService.warning(`Advertencia: Los assets para la versi\xF3n ${i.version} pueden no estar completamente descargados, pero se intentar\xE1 iniciar el juego.`);
    }
    ...
    const getJavaVersionFromPath = (path16) => {
      const match = path16.match(/java(\d+)/i);
      if (match && match[1]) {
    ...
    logProgressService.error(`Error al lanzar la instancia ${i.name}: ${error.message}`, {
      instanceId: i.id,
      error: error.message
    });
    throw error;
...
import_electron2.ipcMain.handle("instance:create-full", async (_e, payload) => {
  try {
    logProgressService.info(`Iniciando creaci\xF3n de instancia: ${payload.name}`, {
      name: payload.name,
      version: payload.version,
    ...
    logProgressService.error(`Error creando instancia ${payload.name}: ${error.message}`, {
      name: payload.name,
      error: error.message
    });
    throw error;
...
import_electron2.ipcMain.handle("instances:scan-and-register", async () => {
  try {
    const { instancesBaseDefault } = basePaths();
    if (!import_node_fs14.default.existsSync(instancesBaseDefault)) {
      console.log("No existe el directorio de instancias:", instancesBaseDefault);
      return { count: 0, registered: [] };
    }
    const allItems = import_node_fs14.default.readdirSync(instancesBaseDefault);
    const instanceFolders = allItems.filter((item) => {
      const itemPath = import_node_path15.default.join(instancesBaseDefault, item);
      return import_node_fs14.default.statSync(itemPath).isDirectory();
    });
    ...
    const unregisteredFolders = instanceFolders.filter((folder) => {
      const folderPath = import_node_path15.default.join(instancesBaseDefault, folder);
    ...
    for (const folder of unregisteredFolders) {
      const instancePath = import_node_path15.default.join(instancesBaseDefault, folder);
      try {
        const instanceJsonPath = import_node_path15.default.join(instancePath, "instance.json");
        let instanceConfig = null;
        if (import_node_fs14.default.existsSync(instanceJsonPath)) {
          instanceConfig = JSON.parse(import_node_fs14.default.readFileSync(instanceJsonPath, "utf-8"));
          if (!instanceConfig.id) {
            instanceConfig.id = `${folder}_${Date.now()}`;
          }
        } else {
          const filesInDir = import_node_fs14.default.readdirSync(instancePath);
          let version = "unknown";
          let loader = "vanilla";
...
import_electron2.ipcMain.handle("instance:install-content", async (_e, payload) => {
  try {
    const isCurseForgeContent = payload.contentId && (payload.contentId.startsWith?.("cf_") || /^\d+$/.test(payload.contentId));
    if (payload.contentType === "modpack") {
      ...
      if (isCurseForgeContent) {
        console.log(`Descargando contenido de CurseForge - ID proyecto: ${payload.contentId}, ID versi\xF3n: ${payload.versionId}`);
        await curseforgeService.downloadContent(
          payload.contentId,
          payload.instancePath,
          payload.mcVersion,
        ...
        console.log(`Descargando contenido de Modrinth - ID proyecto: ${payload.contentId}, ID versi\xF3n: ${payload.versionId}`);
...
import_electron2.ipcMain.handle("instance:is-complete", async (_e, instancePath) => {
  return instanceCreationService.isInstanceComplete(instancePath);
});
import_electron2.ipcMain.handle("downloads:get-all", async () => {
  return downloadQueueService.getAllDownloads();
});
import_electron2.ipcMain.handle("downloads:get-status", async (_e, downloadId) => {
  return downloadQueueService.getDownloadStatus(downloadId);
});
import_electron2.ipcMain.handle("downloads:cancel", async (_e, downloadId) => {
...
import_electron2.ipcMain.handle("java:get-all", async () => {
  return javaService_default.getAllJavas();
});
import_electron2.ipcMain.handle("java:get-default", async () => {
  return javaService_default.getDefaultJava();
});
import_electron2.ipcMain.handle("java:set-default", async (_event, javaId) => {
  return javaService_default.setDefaultJava(javaId);
});
import_electron2.ipcMain.handle("java:remove", async (_event, javaId) => {
  return javaService_default.removeInstalledJava(javaId);
});
import_electron2.ipcMain.handle("java:test", async (_event, javaPath) => {
  try {
    const isWorking = await javaService_default.testJava(javaPath);
    return { isWorking };
  } catch (error) {
...
var pipelineAsync5 = (0, import_node_util5.promisify)(import_node_stream5.pipeline);
...
import_electron2.ipcMain.on("download:start", async (event, { url, filename, itemId }) => {
  ...
  ensureDir2(downloadPath);
  const filePath = import_node_path15.default.join(downloadPath, cleanFilename);
  const fileDir = import_node_path15.default.dirname(filePath);
  ensureDir2(fileDir);
  try {
    ...
    const fileStream = import_node_fs14.default.createWriteStream(filePath);
    const progressStream = new (require("stream")).Transform({
      transform(chunk, encoding, callback) {
...
import_electron2.ipcMain.handle("download:cancel", async (_event, downloadId) => {
  ...
  return downloadQueueService2.cancelDownloadById(downloadId);
});
import_electron2.ipcMain.handle("download:cancelAll", async (_event) => {
  const { downloadQueueService: downloadQueueService2 } = await Promise.resolve().then(() => (init_downloadQueueService(), downloadQueueService_exports));
  return downloadQueueService2.cancelAllDownloads();
});
import_electron2.ipcMain.handle("logs:get-recent", async (_event, count = 50) => {
  return logProgressService.getRecentLogs(count);
});
import_electron2.ipcMain.handle("logs:get-by-type", async (_event, { type, count }) => {
  return logProgressService.getLogsByType(type, count);
});
import_electron2.ipcMain.handle("logs:get-stats", async () => {
  return logProgressService.getStats();
});
import_electron2.ipcMain.handle("progress:get-all-statuses", async () => {
  return logProgressService.getAllProgressStatuses();
});
import_electron2.ipcMain.handle("progress:get-overall", async () => {
  return logProgressService.getOverallProgress();
});
import_electron2.ipcMain.handle("progress:get-download-statuses", async () => {
